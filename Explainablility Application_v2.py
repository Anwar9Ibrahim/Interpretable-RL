...
B_next = Button(root, text ="Run", height=2, width=4, background="green", padx=5, pady=5, command = Run)
B_next.pack()

B_back = Button(root, text ="Back", height=2, width=4, background="pink", padx=5, pady=5, command = Back)
B_back.pack()
...
def get_images(ex_b,renders,episode , row):
    ...
    saliency_a= ex_b.episodes[episode][0][row]["heatmap"]
...
def memory_row(episode , row):
    ...
    info_label= Label(root, text="Episode_"+str(i)+ "  "+ output+" our agent choose the action "+str(action)+" got the reward of "+str(reward)+ " and the total score at this episode is "+str(score) ,background="white", font=("Arial", 15))
    info_label.pack()
    ...
    im_s=plt.imshow(saliency_a[0][0])
    plt.title("Saliency_1")

    fig.add_subplot(rows, columns, 2)
    im_s=plt.imshow(saliency_a[0][1])
    plt.title("Saliency_2")

    fig.add_subplot(rows, columns, 3)
    im_s=plt.imshow(saliency_a[0][2])
    plt.title("Saliency_3")

    fig.add_subplot(rows, columns, 4)
    im_s=plt.imshow(saliency_a[0][0])
    plt.title("Saliency_4")

    fig.add_subplot(rows, columns, 5)
    im_s=plt.imshow(state_a[0][0])
    plt.title("State_1")

    fig.add_subplot(rows, columns, 6)
    im_s=plt.imshow(state_a[0][1])
    plt.title("State_2")

    fig.add_subplot(rows, columns, 7)
    im_s=plt.imshow(state_a[0][2])
    plt.title("State_3")
    ...
    plt.title("Statet_4")
    for coun in range(4):
        fig.add_subplot(rows, columns, 9+coun)
        im_s_= plt.imshow(state_a[0][coun])#, cmap="gray")
        im_s_a= plt.imshow(
            saliency_a[0][coun],
            #norm=colors.TwoSlopeNorm(vcenter=0),
            cmap="jet",
        ...
        plt.title("saliency map, on top of input _"+ str(coun))
        plt.axis("off")
    ...
    ax_= fig.add_subplot(rows, columns, 15)
